...
public class ElectricityBillMasterDao extends AbstractDataAccessObject {
	public boolean insertElectricityBillMaster(ElectricityBillMasterForm cmform) {
		...
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("insert into ElectricityBillMaster values(?,?,?,?,?,?,?,?)");
			ps.setInt(1, n);
	...
	public CoreList ViewElectricityBillMaster(int userid) {
		...
		try {
			...
			ResultSet rs = st
					.executeQuery("select * from ElectricityBillMaster where userid="
							+ userid);
	...
	public ElectricityBillMasterForm ViewElectricityBillMasterById(int BillId) {
		...
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("select * from ElectricityBillMaster where BillId = ? ");
			ps.setInt(1, BillId);
	...
	public boolean deleteElectricityBillMaster(int BillId) {
		...
		try {
			...
			PreparedStatement ps = con
					.prepareStatement("delete from ElectricityBillMaster where BillId = ? ");
			ps.setInt(1, BillId);
			int i = ps.executeUpdate();
			if (i > 0)
				flag = true;
	...
	public boolean UpdateElectricityBillMaster(ElectricityBillMasterForm cmform) {
		...
		try {
			...
			con = getConnection();
			PreparedStatement ps = con
					.prepareStatement("update ElectricityBillMaster set BillDate = ?,userid = ?, billno = ?,serviceno = ?,noofunits = ?,amount = ?,employeid = ? where BillId = ? ");
