...
public class CountryMasterDao extends AbstractDataAccessObject 
{
	public boolean insertCountryMaster(CountryMasterForm cmform)
	{
		...
		con=getConnection();
		PreparedStatement ps=con.prepareStatement("insert into CountryMaster values(?,?,?,?,?,?)");
		ps.setInt(1,n);
...
public CountryMasterForm ViewCountryMasterById(String storepath,int CountryId)
{   CountryMasterForm cmform=new CountryMasterForm();
	...
	try
	{
		...
		PreparedStatement ps=con.prepareStatement("select * from CountryMaster where CountryId = ? ");
...
public boolean deleteCountryMaster(int CountryId)
	{
		...
		try
		{
			...
			PreparedStatement ps=con.prepareStatement("delete from CountryMaster where CountryId = ? ");
			ps.setInt(1,CountryId);
			int i=ps.executeUpdate();
			if(i>0)
				flag=true;
...
public boolean UpdateCountryMaster(CountryMasterForm cmform)
{
	...
	con=getConnection();
	PreparedStatement ps=con.prepareStatement("update CountryMaster set Countryname = ?,abbr = ?, Countrymap = ?,countryarea = ?,description = ? where CountryId = ? ");
...
public String getCountryName(int countryid)
{
	...
	try{
		con=getConnection();
		PreparedStatement pst=con.prepareStatement("Select countryname from countrymaster where countryid=?");
...
{   CoreList v=new CoreList();
	...
	try
	{
		con=getConnection();
		Statement st=con.createStatement();
		ResultSet rs=st.executeQuery("select countryid,countryname from CountryMaster");
