...
{   CoreList v=new CoreList();
	...
	try
	{
		...
		ResultSet rs=st.executeQuery("select pm.Goodsname,cm.countryname,pd.Goodsid,pd.countryid,pd.itemid,pd.itemname,pd.itemimage,pd.itemregdate,pd.itemlaunchvideo,pd.itemdesc,pd.price from Goodsmaster pm,countrymaster cm,Goodsdetails pd where pd.Goodsid=pm.Goodsid and pd.countryid=cm.countryid");
...
{   GoodsDetailsForm pdform=new GoodsDetailsForm();
	...
	try
	{
		...
		PreparedStatement ps=con.prepareStatement("select * from GoodsDetails where ItemId = ? ");
		ps.setInt(1,ItemId);
		...
		if(rs.next())
		{
			...
			OutputStream fout1=new FileOutputStream(storepath+"/"+id+".wav");
			fout1.write(b3);
...
public boolean deleteGoodsDetails(int ItemId)
	{
		...
		try
		{
			...
			PreparedStatement ps=con.prepareStatement("delete from GoodsDetails where ItemId = ? ");
			ps.setInt(1,ItemId);
			int i=ps.executeUpdate();
			if(i>0)
				flag=true;
...
public boolean UpdateGoodsDetails(GoodsDetailsForm pdform)
{
	...
	con=getConnection();
	PreparedStatement ps=con.prepareStatement("update GoodsDetails set itemregdate = ?,Goodsid = ?, countryid = ?,itemname = ?,itemimage = ?,ITEMLAUNCHVIDEO = ?,itemdesc = ?,price = ? where ItemId = ? ");
...
public CoreList ViewGoodsDetails(String storepath,int Goodsid,int countryid)
{   CoreList v=new CoreList();
	java.sql.Connection con=null;
	GoodsDetailsForm pdform=null;
	try
	{
		con=getConnection();
		Statement st=con.createStatement();
		ResultSet rs=st.executeQuery("select pm.Goodsname,cm.countryname,pd.Goodsid,pd.countryid,pd.itemid,pd.itemname,pd.itemimage,pd.itemdesc,pd.price from Goodsmaster pm,countrymaster cm,Goodsdetails pd where pd.Goodsid=pm.Goodsid and pd.countryid=cm.countryid and pd.Goodsid="+Goodsid+" and pd.countryid="+countryid+"");
...
public String getItemname(int itemid)
{
	...
	try{
		con=getConnection();
		PreparedStatement pst=con.prepareStatement("Select itemname from Goodsdetails where itemid="+itemid);
...
public int getItemPrice(int itemid)
{
	...
	try{
		con=getConnection();
		PreparedStatement pst=con.prepareStatement("Select price from Goodsdetails where itemid="+itemid);
...
public CoreHash listItems()
{
	...
	try 
	{
		...
		ResultSet rs = st.executeQuery("select itemid,Goodsid,itemname,itemdesc from Goodsdetails order by Goodsid");
		while(rs.next())
		{
			itemid = rs.getInt(1);
...

{
	...
	try{
		con=getConnection();
		PreparedStatement pst=con.prepareStatement("select price from Goodsdetails where itemid=?");
		pst.setInt(1,itemid);
		ResultSet rs=pst.executeQuery();
		while(rs.next())
		{
